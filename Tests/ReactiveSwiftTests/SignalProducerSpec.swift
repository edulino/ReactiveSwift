...
class SignalProducerSpec: QuickSpec {
	override func spec() {
		...
		describe("then") {
			...
			it("should available to use contextual lookup for arbitrary error signal then same value same error signal") {
				_ = SignalProducer<Int, TestError>.empty
					.then(.init(value: 0))
			}

			it("should available to use contextual lookup for arbitrary error signal then other value same error signal") {
				_ = SignalProducer<Int, TestError>.empty
					.then(.init(result: Result<String, TestError>(value: "")))
			}

			it("should available to use contextual lookup for arbitrary error signal then other value NoError signal") {
				_ = SignalProducer<Int, TestError>.empty
					.then(.init(value: ""))
			}

			it("should available to use contextual lookup for NoError signal then same value same error signal") {
				_ = SignalProducer<Int, NoError>.empty
					.then(.init(value: 0))
			}

			it("should available to use contextual lookup for NoError signal then other value same error signal") {
				_ = SignalProducer<Int, NoError>.empty
					.then(.init(value: ""))
			}

			it("should available to use contextual lookup for NoError signal then other value arbitrary error signal") {
				_ = SignalProducer<Int, NoError>.empty
					.then(.init(result: Result<String, TestError>(value: "")))
			}
		}
